...
uniform float g_BarCount; // {"material":"Number of bars","default":16,"range":[1, 1000]}
uniform float g_Offset; // {"material":"Offset","default":0,"range":[-1, 1]}
uniform vec3 g_BarColor; // {"material":"Color","type":"color","default":"1 1 1"}
...
//TODO: Move duplicate circle code into a "#if SHAPE == CIRCLE_INNER || SHAPE == CIRCLE_OUTER" when logical operators are supported
	...
	vec2 shapeCoord;
	shapeCoord.x = (atan(circleCoord.y / circleCoord.x) + M_PI) / M_PI_2;
